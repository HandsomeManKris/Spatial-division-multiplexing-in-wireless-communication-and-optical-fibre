...
Mr = 1;                     % the number of receive antenna
...
%detection
for ii=1:L_SNR 
   for j = 1 : size(x,1)                                 % The iteratrive maximum ration combining ,the receiver iteratively computes the MRC results and we will use
       for jj=1:Mt                                       % transmit index which has the biggest correlation value with the channel column vectors.
...
for ii=1:L_SNR 
    for j = 1 : size(x,1) 
       bit_Tdem(j,ii*Nt-(Nt-1):ii*Nt)=  de2bi(x_index(ii,j)-1,log2(Mt),'left-msb'); % This MATLAB function converts a nonnegative decimal integer d to a binary row vector.
    end
end
...
semilogy(SNR,ber,'-c','LineWidth',1); hold on
clear; clc
M = 16;  %the number of mudualtion constellation 
Mt = 64;% the nubmer of transmit antenna
...
semilogy(SNR,ber,'-b','LineWidth',1); hold on
clear; clc
M = 16;  %the number of mudualtion constellation 
Mt = 64;% the nubmer of transmit antenna
...
semilogy(SNR,ber,'-k','LineWidth',1); hold on
clear ;   
clc;
M = 4;
...
semilogy(SNR,ber,'--r','LineWidth',1);hold on
clear ;   
clc;
M = 64;
...
title('5 bis tranmission Bit Error Rate of SM using ML detector')
legend('64X1 SSK','64X2 SSK','64X4 SSK','MMSE-MIMO  4QAM 3X4 ','MMSE-MIMO 64QAM 1X1' ,  'Location','NorthEast')
toc