...
for ii=1:L_SNR 
   for j = 1 : Nbits
       x_detected(ii*Mr-(Mr-1):ii*Mr,j)=inv(eye(Mr,Mt)+No(ii)*eye(Mr,Mt))*channel(ii*Mr-(Mr-1):ii*Mr,j*Mt-(Mt-1):j*Mt)'*y(ii*Mr-(Mr-1):ii*Mr,j);
   end
...
semilogy(SNR,ber,'color',[0,0.75,0.75],'linestyle','-','LineWidth',1);hold on

grid on;
ylim([10^(-5) 10^(0)])
xlabel('$$SNR$$','Interpreter','latex')
ylabel('BER','Interpreter','latex')
title('Bit Error Rate of generalized SM using ML detector')
legend('GSM MMF  4 8 16QAM 5X5channel  activate 2 antenna' ,  'Location','NorthEast')       
 toc       