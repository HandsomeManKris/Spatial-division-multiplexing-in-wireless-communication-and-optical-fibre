...
for ii=1:L_SNR 
   for j = 1 : Nbits
       h=channel(ii*Mr-(Mr-1):ii*Mr,j*Mt-(Mt-1):j*Mt);
       x_detected(ii*Mt-(Mt-1):ii*Mt,j)=inv(h'*h+No(ii)*eye(Mt,Mt))*h'*y(ii*Mr-(Mr-1):ii*Mr,j);%MMSE
   end
   x_detect(ii,:)=reshape(x_detected(ii*Mt-(Mt-1):ii*Mt,:),1,Nbits*Mt);
end
...
semilogy(SNR,ber,'-go','LineWidth',1.5);hold on
...
title(' MIMO BER using MMSE detector ')
legend('16QAM 4X4 channel' ,'16PSK 4X4 channel' ,'16QAM 2X8 channel' ,'16PSK 2X8 channel' ,'16QAM 8X2 channel' ,'16PSK 8X2 channel' , 'Location','SouthEast')       
 toc       