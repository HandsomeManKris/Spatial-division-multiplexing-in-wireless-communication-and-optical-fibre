...
clc;
M = 32;
Mt = 10; %for number of transmit antenna
Mr = 10;%for number of receive antenna
Nu=5;
...
for ii=1:L_SNR 
   for j = 1 : size(x,1) 
       ...
       % [x_max,ind] = max(x_detection(:));
        ...
        %x_detected(ii,j)=z(g);
        x_detected(ii,j)=(channel(ii*Mr-Mr+1:ii*Mr,j*Mt-Mt+d)'+channel(ii*Mr-Mr+1:ii*Mr,j*Mt-Mt+f)'+channel(ii*Mr-Mr+1:ii*Mr,j*Mt-Mt+g)'+channel(ii*Mr-Mr+1:ii*Mr,j*Mt-Mt+u)'+channel(ii*Mr-Mr+1:ii*Mr,j*Mt-Mt+r)')*y(ii*Mr-Mr+1:ii*Mr,j)/Nu;
...
clc;
M = 16;
Mt = 64;                         %for number of transmit antenna
Mr = 64;     
Nk=1;%for number of receive antenna
bit_SMsym = log2(M*Mt);         % number of bit per spatial modulation sysmbol
Nbits =5000;
...
legend('MMF GSM 32QAM (10,5) ','MMF GSM 256QAM (6,3) ','MMF SM 64QAM 16x16 ','MMF SM 16QAM 64X64 ' ,'MMF MMSE-MIMO 4QAM 6X6 ','MMF ZF-MIMO 4QAM 6X6 ','Location','NorthEast')
toc