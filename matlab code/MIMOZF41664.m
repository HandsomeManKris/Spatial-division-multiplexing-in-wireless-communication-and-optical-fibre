...
for ii=1:L_SNR 
   for j = 1 : Nbits
       ...
       x_detected(ii*Mt-(Mt-1):ii*Mt,j)=inv(h'*h)*h'*y(ii*Mr-(Mr-1):ii*Mr,j);
   end
...
semilogy(SNR,ber,'-ks','LineWidth',1.5);hold on
...
semilogy(SNR,ber,'--bs','LineWidth',1.5);hold on
clear;clc
M = 16;
...
semilogy(SNR,ber,'-k+','LineWidth',1.5);hold on
...
semilogy(SNR,ber,'--bo','LineWidth',1.5);hold on

grid on;
ylim([10^(-5) 10^(0)])
xlabel('$$SNR$$','Interpreter','latex')
ylabel('BER','Interpreter','latex')
title(' MIMO BER using ZF detector ')
legend('4QAM 8X8 channel' ,'4PSK 8X8 channel' ,'16QAM 4X4 channel' ,'16PSK 4X4 channel' ,'64QAM 2X2 channel' ,'64PSK 2X2 channel' , 'Location','SouthWest')       
 toc       